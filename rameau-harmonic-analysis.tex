...
\begin{abstract}
  ...
  140 Bach chorales using precision and recall, and discuss
  improvements.
...
\cite{pardo.ea:automated} state that ``no researchers have published
...
information is relevant. For example, in \cite{pardo.ea:algorithms} a
German augmented sixth chord is mistakenly identified as a dominant
seventh chord. Most systems, such as the one found in
\cite{temperley.ea:modeling}, either ignore this problem or develop a
pitch speller to correct the output. Most harmonic analysis software,
with the notable exception of MusEs \cite{pachet.ea:representing} and
the system described in \cite{illescas.ea:harmonic}, uses input
...
algorithm implemented in rameau at present makes direct use of it yet,
since they are reimplementations of algorithms that expect pitch
classes as input.
...
Herrera \cite{gomez.ea:song} do for tonality recognition.

Rameau is written in Common Lisp and runs on the Steel Bank Common
Lisp \cite{team:sbcl} and CMUCL \cite{maclachlan:cmucl} compilers and
the Clisp \cite{haible.ea:clisp} interpreter on the GNU/Linux and
...
Rameau performs the analysis of one chorale, using all algorithms, in
less than a half-second on a Pentium Celeron M at 1.4 GHz with 2 GB
RAM linux box. All 140 chorales are analyzed and the results compared
with the answer sheet under 40 seconds.
...
the Kostka-Payne \cite{kostka.ea:tonal} corpus, Beethoven sonatas,
...
possible to usual popular notation, represent inherent ambiguities in
analysis, and represent non-chord tones.
...
Melisma Music Analyzer and described in \cite{temperley.ea:modeling}.
...
performance claimed in the article. Tsui \cite{tsui:harmonic} was also
...
The A in partition 36 is a retardation, the C in p. 37 a passage, the
G in p. 38 another retardation, and the E in p. 39 a cambiata. The
...
unimplemented in Rameau. We plan on implementing segmentation, which
will allow us to extend our corpus of music and answer sheets,
incorporating, for example, the Kostka-Payne corpus
\cite{temperley:bayesian}. We are looking for new metrics and
evaluation techniques, and will extend and improve our existing
algorithms accordingly. We also intend to implement functional and
non-chord tone analysis, and modify the existing algorithms to use the
base-96 codification.
