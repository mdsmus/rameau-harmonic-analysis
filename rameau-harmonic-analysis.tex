...
%about 150-200 words.
  ...
  music research problem. Although mane have proposed solutions and
...
notes, rhythm, dynamics, harmony, and because music is not a
linear process, unlike image \cite{mouton.ea:numeric} \nota{iara:``and
  by the fact that music, unlike image, is not a linear process.''
  ficaria melhor que ``and by the fact that music is not a linear
  process, unlike image.''} . The harmonic changes in a choral
...
among them \nota{ iara:o ``among them'' me soou muito como a tradução
  de ``entre eles''. Talvez ``such as'' ficasse melhor? }, arranging,
detection of possible logical mistakes in scores, database search,
automatic accompaniment generation, and statistical analysis of
musical styles for automated composition
...
so \nota{iara: eu colocaria um ``are'' nesse lugar }, thus very difficult to
...
and provide objective results, such as error rates or accuracy
degree. \nota{ iara: eu entendi que vcs estavam listando coisas que o
  sistema Blá Blá Blá não faz (né?), mas isso ficaria mais claro se
  vcs juntassem as tres frases com negativas (por exemplo, ao invés de
  ``and provide'', ``or provide'' - cause i'm guessing they don't
  provide objective results... or did i get it wrong? )  }
...
dividing it into four sub-problems. \nota{iara: Eu diria que ``the
  problem of automated harmonics analysis may be understood'', e não
  ``might be understood'', já que vcs parecem estar mais falando de um
  fato que de uma possibildade. (eu sei que may tb é possibiliade, mas
  é mais um pode de fato... but this is silly). Eu tb diria ``may be
  better understood if we divide it into 4 sub-problems'', ao inves de
  ``by dividing it into 4 sub-problems'' (but this is still silly). }

The first problem is pitch spelling. If sharps and flats are not
distinguished the program may have problems in places where
\nota{iara: ``whenever'' fica melhor que ``in places where''}
enharmonic information is important. Most systems either ignore this
problem or develop a pitch speller \cite{temperley.ea:modeling}. To
our knowledge most harmonic analysis software, with the notable
exception of the system described in \cite{illescas.ea:harmonic} and
the MusEs \cite{pachet.ea:representing} system, uses input formats
that lose enharmonic information.
...
\nota{iara: fica feio começar com ``probably''. Não seria melhor ``The
  picth-class notation, or some variation such as MIDI, is probably
  the most used....''? Tb deu vontade de pensar ``in order to
  adress'', e acho que depois de base-40 não tem vírgula.}Probably the
pitch-class notation, or some variation such as MIDI, is the most used
way to numerically represent pitch. The main problem with this
notation is the loss of enharmonic spelling. In this section we will
revise three notations created independently by Brinkman, Hewlett, and
Oliveira \cite{brinkman:binomial, hewlett:base-40, oliveira:busca} to
address this problem.  For the sake of simplicity we will refer to them as
...
two. \nota{iara: ``As'' é mais bonitinho que ``Since''. E simplesmente
  ``comparing with the other two'' ficou estranho ( ou ``comparing it
  with the other two'' ou ``in comparison with the other two'').}

The base-96 is also a single number notation. It has all the
advantages of the base-40 notation with some additional advantages
\nota{iara: ``"it has all the qualities of the base-40 notation with a
  few advantages'', ou algo assim? Tb acho que a frase depois de Table
  3 não precisa começar com ``in this table'', já está claro de que
  table vcs estão falando. (e acho que o p de ``p, M, m, d, A'' era para
  ter sido maiusculo, não? Pq não tem p na Table, só P)}.  Table
...
The problem with the base-40 notation is that it has problems
\nota{iara: Parece que vcs estao dizendo, in short, que ``the problem
  is that is has problems''. Não tem como substituir o segundo problems
  por algo como ``não lida bem com'' em ingles, ou sei lá? (not the best
  sugestion, huh? ) }with more than two accidentals. For example, the
interval between C$\flat\flat$ and C$\sharp\sharp$ (a quadruple
augmented unison) has the same value (6) as diminished second. On the
other hand, in the base-96 notation this interval can be correctly
computed.
...
while in the binomial system it has to use \nota{iara: ``requires'' é
  mais bonitinho que ``it has to use''} two different operations (mod
...
one for each input sonority. Chords are structures that may have a
root, a mode, a seventh, a bass, and other additions. The framework
...
``—''. \nota{o único exemplar neste exemplo está no primeiro acorde do
  Context Tree e evidentemente incorreto. Como distinguir o correto do
  incorreto no caso das non-chord tones?} It is important to remember
that while some algorithms identify chords, others identify only the
root. \nota{jama: porque no Temper.-Sle. todos os acordes estão
  errados (bold italic) inclusive os três primeiros?}
...
\item Although they have been used in many researches
  \cite{taube:automatic, tsui:harmonic, illescas.ea:harmonic,
    winograd:linguistics}, none of them have analyzed all of
  them. \nota{iara: a troca de sujeito deixa a frase um pouco
    estranha, fora que ``none of them have analyzed all of them'' ta a
    bit ugly. Que tal ``Although they have been used in may
    researches, they have not been all analyzed'' ou algum outro fim
    de frase?}
\end{enumerate}
...
used to render the scores. \nota{iara: esse período está muito
  confuso. ``has tools to convert from different formats (such as
  MIDI) to it'' eu nao entendi bem}
